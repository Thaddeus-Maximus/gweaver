...
class DXFEntity(object):
  ...
  def __repr__(self):
    return "DXFEntity from %s to %s" % (repr(self.start), repr(self.end))

def dxf_entities(filename, scaling=1.0):
  "Stitches DXF entities. Assumes a DXF in the X-Y plane of only lines, arcs, and splines."
  EPSILON = 1e-5

  doc = dxf.readfile(filename)
  msp = doc.modelspace()
  ...
  while len(segs) > 0:
    for i, seg in enumerate(segs):
      ...
      for i, seg in enumerate(segs):
        ...
        raise Exception("Failed to connect segment.")
        break

  return segs_stitched
